...
mailsplitDissector::mailsplitDissector():EvidenceDeriveAccessor("mailsplit","default") {
...
void mailsplitDissector::processEvidence ()
{
  FILE *command;
  char *line;
  size_t size;
  ...
  snprintf (cline, 1023,
	    "%s %s/sbin/mailsplit.pl %s %s 2>/dev/null",
...
main (int, char *[])
{
  mailsplitDissector *mailsplit = 0;
  try
  {
    mailsplit = new mailsplitDissector ();
  ...
  {
    mailsplit->getLogStream (LOG_CRIT) << "Ocfa Exeption cougth: " << ex.
      what () << "\n";
  }
  catch (...)
  {
    mailsplit->getLogStream (LOG_CRIT) << "Non Ocfa Exeption cougth\n";
  }
  mailsplit->PrintObjCount ();
  delete mailsplit;
  return 1;
}
