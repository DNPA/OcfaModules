...
#include <iostream>
#include <fstream>
#include "w32proxy.hpp"

using namespace std;
using namespace ocfa::evidence;
using namespace ocfa::misc;

W32Proxy::W32Proxy(std::string modulename) : ocfa::facade::EvidenceDeriveAccessor(modulename,"default"), d_thorworkdir(""), d_thorresultdir("") {

  //Read configuration entries
  std::string w32proxybasedir=getConfEntry("w32proxyworkdir");
  if (w32proxybasedir == "") 
       throw OcfaException("Conf-error: w32proxyworkdir not found in config",this);
  if((mkdir(w32proxybasedir.c_str(),0750)==-1) && (errno != EEXIST)) { 
     throw OcfaException("ERROR creating w32proxyworkdir dir",this); 
  }
  d_thorworkdir = w32proxybasedir + "/" + modulename;
  if((mkdir(d_thorworkdir.c_str(),0750)==-1) && (errno != EEXIST)) {
     throw OcfaException("ERROR creating w32proxyworkdir/module dir",this);
  }

  d_thorresultdir = d_thorworkdir + "/output";
  if((mkdir(d_thorresultdir.c_str(),0750)==-1) && (errno != EEXIST)) {
     throw OcfaException("ERROR creating w32proxyworkdir/module/output dir",this);
  }
  d_thorinfile = modulename + "main.in";
  getLogStream(LOG_DEBUG) << "ThorWorkdir " << d_thorworkdir << endl;
} 


void W32Proxy::processEvidence()
{
  char cline[1024];
  char workfile[1024];
  DIR *wdir = 0;
  ...
  // Create or truncate w32proxy infile for writing
  ofstream out_file;
  ...
  if (out_file.bad() ) {
      throw OcfaException("Unable to write to w32proxy infile.\n");
  }
  out_file << EvidenceBasename;
  out_file.close() ;
 

  // Wait for file *.in to disappear. This should indicate the Thor module
  // finished unpacking the w32proxy file. 
  int i = 0;
  while (fileExists(outfilename)){ 
    ...
    if ((++i % 10) == 0){
         aliveAndKicking(0,0);
	 getLogStream(LOG_DEBUG) << "Waiting " << i << " secs for " << d_thorinfile << " to disappear." << endl;
       }
  }
...
bool W32Proxy::fileExists(const string filename) const {
  struct stat buf;
...
int main(int argc , char **argv)
{
  W32Proxy *w32proxy = 0; 
  if (argc < 2) {
     cerr << "Usage : \n\t" << argv[0] << " <w32modulename>\n\n";
  }
  try {
    w32proxy=new W32Proxy(argv[1]);
  ...
  try {
    ...
    ex.logWhat();
    w32proxy->getLogStream(LOG_CRIT) << "Ocfa Exeption cougth: " << ex.what() << "\n";
  } catch(...) {
    w32proxy->getLogStream(LOG_CRIT) << "Non Ocfa Exeption cougth\n";
  }
  w32proxy->PrintObjCount();
  delete w32proxy;
  return 1;
}
