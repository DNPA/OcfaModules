...
void WordPerfect::processEvidence() {
  FILE *command;
  char *line;
  size_t size;
  char cline[1024];
  ...
  //FIXME: we don't have a manpage for wpd2text, and no wpd files to test with, not sure if this is right.
  int retval = snprintf(cline,1023,"%s %s 2>%s",BINARY_WPD2TEXT,
  ...
  if (command==0) 
    throw OcfaException("Unable to fork: wpd2text -s %s \n");
  size=0;
  line=0;
  ...
  if (errorfil == 0) {
      throw OcfaException("Unable to open wpd2text errorfile for reading\n");
  }
  ...
  ScalarMetaValue encoding(Scalar("latin1")); //FIXME: need to make sure this is the case.
...
int main(int , char *[]){
  WordPerfect *wordperfect=0;
  try {
    wordperfect = new WordPerfect();
  ...
  try {
    ...
    ex.logWhat();
    wordperfect->getLogStream(LOG_CRIT) << "Ocfa Exeption cougth: " << ex.what() << "\n";
  } catch(...) {
    wordperfect->getLogStream(LOG_CRIT) << "Non Ocfa Exeption cougth\n";
  }
  wordperfect->PrintObjCount();
  delete wordperfect;
  return 0;
}
