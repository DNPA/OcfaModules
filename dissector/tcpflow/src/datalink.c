 ...
 * LICENSE for details.
 *
 * $Id: datalink.c,v 1.1 2007/07/24 08:06:08 oscar Exp $
 *
 * $Log: datalink.c,v $
...
static char *cvsid = "$Id: datalink.c,v 1.1 2007/07/24 08:06:08 oscar Exp $";
...
void dl_null(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
  ...
  process_ip(p + NULL_HDRLEN, caplen - NULL_HDRLEN, ts);
}
...
void dl_ethernet(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
  ...
  process_ip(p + sizeof(struct ether_header),
	     caplen - sizeof(struct ether_header), ts);
}
...
void dl_ppp(u_char *user, const struct pcap_pkthdr *h, const u_char *p)
{
  ...
  process_ip(p + PPP_HDRLEN, caplen - PPP_HDRLEN, 0);
}
...
void dl_linux_sll(u_char *user, const struct pcap_pkthdr *h, const u_char *p){
  ...
  process_ip(p + SLL_HDR_LEN, caplen - SLL_HDR_LEN, 0);
}
