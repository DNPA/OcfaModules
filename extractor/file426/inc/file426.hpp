...
namespace ocfa {
 namespace evidence {
   class jFile : public ocfa::facade::EvidenceFileAccessor {
	 ...
	 string fileMagicMgc = ocfaRoot + "/share/file/magic.mgc";
         string fileMagic = ocfaRoot + "/share/file/magic";
	 //string mimeFileAcc = mimeFile + ".mime";
	 if(access(fileMagicMgc.c_str(), R_OK) < 0) {
	     throw OcfaException("No read access on magic.mgc file", this);
	 }
       
         //Open and load Magic cookie for magic mime_type
         d_magic_mime = magic_open(MAGIC_MIME_TYPE | MAGIC_SYMLINK);
	 if (d_magic_mime == NULL){
           throw OcfaException("Magic MIME cookie intialization failed.",this);
	 }
	 if (magic_load(d_magic_mime,fileMagic.c_str()) != 0){
           throw OcfaException("Loading of magic MIME file failed.",this); 
	 }

         //Open and load Magic cookie for magic text
	 d_magic_text = magic_open(MAGIC_SYMLINK);
	 if (d_magic_text == NULL){
	    throw OcfaException("Magic TEXT cookie intialization failed.",this);
         }
	 if (magic_load(d_magic_text,fileMagic.c_str()) != 0){
